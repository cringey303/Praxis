...
interface ProjectCardProps {
    ...
    currentUser?: { id: string; major?: string } | null;
}
...
export function ProjectCard({ project, currentUser }: ProjectCardProps) {
    ...
    const canApply = currentUser && currentUser.id !== project.owner_id && project.status === 'open';
        ...
        <>
                ...
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    {project.looking_for && project.looking_for.length > 0 && (
                        project.looking_for.map((major) => (
                            <span key={major} className="px-2 py-1 rounded-full text-xs font-medium border bg-purple-500/10 text-purple-400 border-purple-500/30">
                                {major}
                            </span>
                        ))
                    )}
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-border">
                    ...
                    {canApply && (
                        <button
                            ...
                            className="ml-auto flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors cursor-pointer"
                        >
                            Apply
                        </button>
                    )}
            ...
            {isApplying && (
                <ApplicationModal
                    ...
                    applicantMajor={currentUser?.major}
